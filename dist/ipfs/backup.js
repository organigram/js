...
import { aesKeyAlgorithm, exportUserPrivateKey, getCrypto, getRandomIv, importExtractableUserPublicKey, importUserPrivateKey } from './crypto';
import { base64ToBytes, bytesToArrayBuffer, bytesToBase64, bytesToText, textToBytes } from './encoding';
...
const deriveBackupEncryptionKey = async (passphrase, salt, iterations = backupKeyIterations) => {
    const passphraseKey = await getCrypto().subtle.importKey('raw', bytesToArrayBuffer(textToBytes(passphrase)), 'PBKDF2', false, ['deriveKey']);
    return await getCrypto().subtle.deriveKey({
        name: 'PBKDF2',
        hash: 'SHA-256',
        salt: bytesToArrayBuffer(salt),
        iterations
    }, passphraseKey, aesKeyAlgorithm, false, ['encrypt', 'decrypt']);
};
export const exportUserEncryptionKeyBackup = async (address, passphrase) => {
    if (passphrase === '') {
    ...
    const ciphertext = await getCrypto().subtle.encrypt({ name: 'AES-GCM', iv: bytesToArrayBuffer(iv) }, backupKey, bytesToArrayBuffer(textToBytes(JSON.stringify(privateKey))));
    return {
...
export const importUserEncryptionKeyBackup = async (backup, passphrase) => {
    ...
    const backupKey = await deriveBackupEncryptionKey(passphrase, salt, backup.privateKey.iterations);
    const plaintext = await getCrypto().subtle.decrypt({ name: 'AES-GCM', iv: bytesToArrayBuffer(iv) }, backupKey, bytesToArrayBuffer(base64ToBytes(backup.privateKey.ciphertext)));
    const privateKey = await importUserPrivateKey(JSON.parse(bytesToText(plaintext)));
