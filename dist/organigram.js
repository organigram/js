...
class Organigram {
    ...
    static async loadProcedureTypes(address, provider) {
        ...
        const procedureTypes = await Promise.all(procedures.map(async (procedure) => await Organigram.loadProcedureType({
    ...
    static async load(address, provider, signer) {
        ...
        const procedureTypes = await Organigram.loadProcedureTypes(address, provider);
        const chainId = await provider
            ?.getNetwork()
            .then(n => n.chainId.toString())
            .catch(() => '');
        const newOrganigram = new Organigram(address, chainId, procedureTypes, contract, provider, signer);
